...
function getPdfUrlFromSR(sr) {
  ...
  if (sr.pdfPath) {
    const fileName = String(sr.pdfPath).split(/[\\/]/).pop();

    if (fileName) {
      return `/outbound/requisitions/document/${encodeURIComponent(
        fileName
      )}`;
    }
  }
  ...
  if (sr.srNumber) {
    const safeSrNumber = String(sr.srNumber).replace(/[^\w\-ก-๙]/g, "");
...
export default function StockRequisitionListPage() {
  ...
  const handleOpenPdf = async (sr) => {
    const pdfUrl = getPdfUrlFromSR(sr);
    ...
    setOpeningPdfId(sr.id);

    try {
      const url = buildBackendFileUrl(pdfUrl);
      const token =
  ...
  const getStatusBadge = (status) => {
    const baseClass =
      "px-3 py-1.5 rounded-md text-[11px] font-bold uppercase tracking-wider border shadow-sm flex items-center gap-1.5 w-fit";
    ...
    switch (status) {
      ...
      case "REJECTED":
        return (
          <span
            className={`${baseClass} bg-rose-50 text-rose-600 border-rose-100`}
          >
      ...
      case "DISPATCHED":
        return (
          <span
            className={`${baseClass} bg-indigo-50 text-indigo-600 border-indigo-100`}
          >
            <Truck className="w-3.5 h-3.5" /> จ่ายของแล้ว
          </span>
        );
      ...
      default:
        return (
          <span
            className={`${baseClass} bg-slate-50 text-slate-600 border-slate-200`}
          >
            {status || "-"}
          </span>
        );
          ...
          <div className="flex flex-row items-center gap-4 w-full md:w-auto">
            {viewMode === "LIST" ? (
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => handleOpenPdf(selectedSR)}
                  disabled={openingPdfId === selectedSR?.id}
                  className="print:hidden inline-flex items-center justify-center gap-2 px-5 py-3 bg-[#1F3B8B] text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-[#172e6d] transition-all shadow-sm disabled:opacity-50"
                >
                  {openingPdfId === selectedSR?.id ? (
                          ...
                          <td className="py-4 px-6 text-right">
                            <div className="flex items-center justify-end gap-3">
                              <button
                                ...
                                className="text-[11px] font-bold text-rose-600 hover:underline flex items-center gap-1 disabled:opacity-50"
                              >
                                {openingPdfId === req.id ? (
                                  <Loader2 className="w-4 h-4 animate-spin" />
                                ) : (
                              ...
                              <button
                                ...
                                className="text-[11px] font-bold text-[#1F3B8B] hover:underline flex items-center gap-1"
                              >
            ...
            <div className="bg-white rounded-xl border-2 border-slate-300 shadow-md overflow-hidden">
                ...
                <DetailInfoItem
                  ...
                  value={
                    selectedSR.approver?.firstName
                      ? `${selectedSR.approver.firstName} ${selectedSR.approver.lastName}`
                      : "-"
                  }
                />
              </div>
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-base text-slate-800 font-semibold leading-relaxed p-5 bg-slate-50 rounded-xl border border-slate-100">
                    {selectedSR.remarks ||
                  ...
                  <table className="w-full border-collapse">
                        ...
                        return (
                          <tr key={idx} className="text-base">
                            ...
                            <td className="p-4 text-center">
                              <span className="px-3 py-1 bg-slate-100 rounded-md font-bold text-[#1F3B8B]">
                                {item.quantity}
                              </span>
                            </td>
                    ...
                    <tfoot className="bg-[#1F3B8B]/5 font-bold">
                      <tr>
                        <td
                          colSpan="4"
                          className="p-5 text-right text-sm uppercase text-slate-600 tracking-wider"
                        >
                          มูลค่าประมาณการสุทธิ
                        </td>

                        <td className="p-5 text-right text-xl text-emerald-600 tabular-nums">
                          ฿{formatCurrency(grandTotalValue)}
                        </td>
                      </tr>
                    </tfoot>
