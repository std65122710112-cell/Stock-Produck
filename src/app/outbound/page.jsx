...
import {
  ...
  Plus,
  Calendar,
  Loader2, // 💡 นำเข้า Plus icon สำหรับปุ่มแบ่งเบิก
} from "lucide-react";
      ...
      {modal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full p-8 md:p-10 overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-300">
            ...
            <div
              className={`mx-auto w-20 h-20 rounded-[1.5rem] flex items-center justify-center mb-6 ${modal.type === "danger" ? "bg-rose-50 text-rose-500" : "bg-blue-50 text-[#1F3B8B]"}`}
            >
              {modal.type === "danger" ? (
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className={`flex-1 px-6 py-4 rounded-2xl font-black text-xs uppercase tracking-widest text-white shadow-lg transition-all active:scale-95 ${
                  modal.type === "danger"
                ...
                <tbody className="divide-y divide-slate-100 bg-white/50">
                  {isLoading ? (
                    <tr>
                      <td
                        colSpan="5"
                        className="p-24 text-center text-slate-400 font-black uppercase text-sm animate-pulse tracking-widest"
                      >
                        กำลังโหลดข้อมูลระบบ...
                      </td>
                    </tr>
                    ...
                    <tr>
                      <td
                        colSpan="5"
                        className="p-32 text-center text-slate-400 font-black uppercase text-xs"
                      >
                    ...
                    allSRs.map((sr) => (
                      ...
                      >
                        <td className="p-6">
                          <span
                            className={`tabular-nums font-black text-base tracking-tight transition-colors ${sr.status === "COMPLETED" ? "text-slate-300" : "text-[#1e3b8a] group-hover:text-blue-800"}`}
                          >
                            {sr.srNumber}
                          </span>
                        </td>
                        <td className="p-6">
                          <p
                            className={`font-black text-sm uppercase ${sr.status === "COMPLETED" ? "text-slate-400" : "text-slate-800"}`}
                          >
                        ...
                        <td className="p-6 text-center">
                          <span
                            className={`inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider border shadow-sm ${sr.status === "COMPLETED" ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-amber-50 text-amber-600 border-amber-200"}`}
                          >
                            {sr.status === "COMPLETED" ? (
                        ...
                        <td className="p-6 text-right">
                          {sr.status === "COMPLETED" ? (
                            <span className="text-xs font-black text-slate-400 uppercase italic pr-4">
                              จ่ายออกสำเร็จ ✓
                            </span>
                          ) : (
                            <button
              ...
              <div className="bg-white p-8 md:p-10 relative overflow-hidden border-b border-slate-100">
                ...
                <div className="relative z-10 flex flex-col md:flex-row gap-6 md:gap-12 items-center">
                  <div>
                    ...
                    <p className="tabular-nums font-black text-3xl md:text-4xl tracking-tight text-emerald-600">
                      {doNo}
                    </p>
                  </div>
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    {/* ข้อมูลเป็นสีเขียว (Green Data) */}
                    <p className="tabular-nums font-black text-xl md:text-2xl tracking-tight text-emerald-600">
                      {selectedSR.srNumber}
                    </p>
                  </div>
                </div>
                ...
                <section className="px-8 md:px-10 py-10 bg-slate-50/50 flex flex-col gap-8 border-b-2 border-slate-100">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div className="flex items-center gap-2 bg-white px-5 py-2.5 rounded-2xl border-2 border-slate-200 shadow-sm">
                      ...
                      <span className="text-xs font-black text-slate-600 uppercase tracking-widest tabular-nums">
                        วันที่ขอเบิก:{" "}
                        {new Date(selectedSR.createdAt).toLocaleDateString(
                          "th-TH",
                        )}
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white p-7 rounded-[2rem] border-2 border-slate-200 shadow-sm hover:shadow-md transition-all">
                      ...
                      <p className="text-lg font-black text-black ml-1">
                    ...
                    <div className="bg-white p-7 rounded-[2rem] border-2 border-slate-200 shadow-sm hover:shadow-md transition-all">
                      ...
                      <p className="text-lg font-black text-emerald-800 ml-1">
                        {selectedSR.approver?.firstName}{" "}
                    ...
                    <div className="bg-white p-7 rounded-[2rem] border-2 border-slate-200 shadow-sm hover:shadow-md transition-all">
                      ...
                      <p className="text-lg font-black text-black ml-1">
                        {selectedSR.department?.name || "ส่วนกลาง (General)"}
                      </p>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-left border-separate border-spacing-y-3 min-w-[900px]">
                      ...
                      <thead>
                        <tr className="text-[13px] font-bold text-slate-500 uppercase tracking-widest">
                          ...
                          <th className="px-6 py-2 w-[35%]">
                            หยิบจากคลัง/โซน (Location){" "}
                                ...
                                {isFirstOfGroup ? (
                                  <div className="flex flex-col">
                                    <p className="font-bold text-slate-900 text-base uppercase tracking-tight">
                                      <span className="text-slate-400 mr-2 tabular-nums font-bold">
                                        #{item.sku}
                                      </span>
                                      {item.productName}
                                    </p>
                              ...
                              <td className="px-6 py-5 text-center border-y border-slate-200/70 group-hover:border-blue-300 group-hover:bg-blue-50/30 transition-colors">
                                {isFirstOfGroup ? (
                                  <div className="flex flex-col items-center">
                                    <span className="font-black text-slate-900 text-2xl tracking-tighter tabular-nums leading-none">
                                      {item.requiredQty}
                                    </span>
                                    <span
                                      className={`text-[10px] font-black px-2 py-0.5 rounded-full mt-2 border tabular-nums shadow-sm
                                ...
                                >
                                  ...
                                  {locs.map((l) => (
                                    <option
                                      ...
                                      value={l.locationId}
                                    >
                                      {l.location.warehouse?.name} | จุดเก็บ:{" "}
                        ...
                        <div className="text-lg font-black tracking-tight flex items-center justify-center md:justify-start gap-2.5">
                          {canSubmit ? (
                            ...
                            <div className="flex items-center gap-2.5">
                              ...
                              <span className="text-amber-600 uppercase text-sm">
